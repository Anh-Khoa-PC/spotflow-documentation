...
:::note[To summarize]
*Payments are processed by creating a transaction using either our API, JavaScript libraries, Inline JS, or SDKs. <span style={{backgroundColor: "yellow", textDecoration: "underline", textDecorationColor: "yellow"}}>Each transaction generates a unique link for payment completion.</span>*
:::
...
- **Spotflow Inline**: Our client-side integration provides a smooth and intuitive checkout experience for your customers. By embedding our JavaScript library directly into your checkout page, you can create a smooth payment flow without redirecting users away from your site. We handle the entire payment process, from initiating the transaction to delivering payment confirmation.

- **Spotflow Classic**: This server-side integration offers greater flexibility and control over the payment process. By utilizing our API, you can generate secure payment links and redirect your customers to our payment page. Once the payment is complete, we'll redirect them back to your site with the transaction details.

**For businesses with custom payment flows**:

- **Direct Charge**: If you prefer to build your own unique payment experience, our Charge API empowers you to create custom payment flows tailored to your business. You have full control over the user interface and payment collection process while leveraging Spotflow's robust payment processing capabilities.
